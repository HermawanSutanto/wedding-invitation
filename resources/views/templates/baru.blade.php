@php
    // --- BLOK DATA DUMMY UNTUK PENGEMBANGAN TEMPLATE ---

    // Nama tamu di halaman sampul
    $guestName = 'Budi dan Keluarga';

    // Membuat objek utama 'invitation'
    $invitation = (object) [
        'id' => 1, // <-- TAMBAHKAN BARIS INI
        'groom_name' => 'Aditya',
        'bride_name' => 'Kirana',
        'cover_image' => 'images/placeholder-cover.jpg', // Ganti dengan path gambar placeholder
        'hero_image' => 'images/placeholder-hero.jpg',   // Ganti dengan path gambar placeholder
        ...
        'groom_photo_path' => 'images/invitations/groom_1756217532.webp', // Ganti dengan path gambar placeholder
        'groom_info' => 'Bapak Subagio & Ibu Wati',
        'bride_photo_path' => 'images/invitations/bride_1756217535.webp', // Ganti dengan path gambar placeholder
        'bride_info' => 'Bapak Sutrisno & Ibu Murni',
        'dress_code_info' => 'Kenakan pakaian terbaik Anda dengan sentuhan warna pastel.',

        // Data Paket Fitur (untuk mengontrol tampilan section)
        'package' => (object) [
            'has_love_story' => true,
        ...
        'events' => [
            (object) [
                ...
                'livestream_link' => 'https://youtube.com/live/yourstreamid', // Link untuk acara akad
                'dress_code_colors' => ['#e3d5d1', '#c8a18f', '#bfa06b', '#8e8d8a'], // Warna dress code hanya di acara pertama
            ],
            (object) [
                ...
                'livestream_link' => null, // Tidak ada livestream untuk resepsi
    ...
    // Menghitung jumlah gambar di galeri
    $galleryCount = count($invitation->galleries);
    $galleryLayoutClass = 'gallery-container'; // Class default
   ...
   @if($invitation->package && $invitation->package->has_music)
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            ...
            // --- LOGIKA BARU UNTUK BUKU TAMU (GUEST BOOK) ---
    const rsvpForm = document.getElementById("rsvp-form");
    const guestbookContainer = document.getElementById("guestbook-container");
    const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');

    // Fungsi untuk menampilkan entri baru di daftar
    const displayGuestbookEntry = (entry) => {
        const entryDiv = document.createElement("div");
        entryDiv.className = "guestbook-entry";
        entryDiv.innerHTML = `
            <div class="header">
                <span class="name">${entry.name}</span>
                <span class="attendance"><i class="fa-solid fa-circle-check"></i> ${entry.attendance_status}</span>
            </div>
            <p class="message">${entry.message}</p>`;
        
        // Hapus pesan "belum ada ucapan" jika ada
        const noWishesEl = guestbookContainer.querySelector('.no-wishes');
        if(noWishesEl) noWishesEl.remove();
    ...
    if (rsvpForm) {
        rsvpForm.addEventListener("submit", function (event) {
            ...
            .then(data => {
                if (data.success) {
                    displayGuestbookEntry(data.entry);
                    this.reset();
